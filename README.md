# Hi, I'm Ishrak Adnan Omi (Bojrodev).

I write software, fine-tune models, and build mobile frontends for local AI tools. Most of my work centers around running open-source models on consumer hardware and creating mobile interfaces to interact with them.

---

### What I'm working on

* **Resolver-WebUI-Client** — An Android client for Stable Diffusion WebUI Forge so I can generate images on my local setup directly from my phone.
* **LoRA & Image Adapters** — Training and tweaking custom adapters for SDXL, Flux, and Zimage Turbo.
* **Local LLMs** — Fine-tuning and merging Llama variants into GGUFs for local runtimes.

---

### Tools & Setup

* **Languages:** Python, JavaScript
* **AI/ML & Mobile:** PyTorch, Unsloth, Capacitor
* **Environment:** Linux (WSL2)

---

### Find me around

[Website](https://bojrodev.com) • [Hugging Face](https://huggingface.co/bojrodev) • [Support / Donate](https://bojrodev.com/donate)

> **Note:** I maintain these projects in my free time and am not taking on freelance work, commissions, or outside collaborations.
